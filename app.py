...
# Streamlit app
def main():
    st.title("Web App Visualisasi Prediksi Harga Saham")

    # Sidebar for data download
    st.sidebar.header("Filter Pencarian Saham")
    stock_symbol = st.sidebar.text_input("Masukan Kode Saham Emiten (e.g., BBNI.JK):", "BBNI.JK")
    start_date = st.sidebar.date_input("Start Date", pd.to_datetime("2019-01-01"))
    end_date = st.sidebar.date_input("End Date", pd.to_datetime("2024-10-07"))
    ...
    model_type = st.sidebar.selectbox("Pilih Tipe Model", ["LSTM", "GRU"])

    # Load saved models
    if model_type == "LSTM":
        final_model = load_model("model_lstm.h5")
    else:
        final_model = load_model("model_gru.h5")
...
def visualize_predictions(data, train_size, n_steps, y_test_orig, y_pred):
    ...
    fig.update_layout(title="Stock Price Prediction",
                      ...
                      yaxis=dict(range=[0, max(y_test_orig)+10/100*max(y_test_orig)])
                      )
